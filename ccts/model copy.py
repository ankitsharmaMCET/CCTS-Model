...
# =====================================================================
# ENHANCED CCTS MODEL - CORRECTED VERSION
# =====================================================================
...
from .market import EnhancedCarbonMarket
...
class CCTSModel(Model):
    ...
    def __init__(self, firm_data: pd.DataFrame, market_params: Dict, model_config: Dict, seed: int = None):
        ...
        self.market = EnhancedCarbonMarket(self, market_params, initial_price)
    ...
    def _setup_data_collection(self) -> None:
        ...
        model_reporters = {
            ...
            "Market_Maker_Cash": lambda m: m.market.market_maker_cash,
            "Market_Maker_Reserve": lambda m: m.market.market_maker_reserve,
        }
    ...
    def step(self) -> None:
        ...
        try:
            # Step the agents and the market
            self.schedule.step()
            self.market.step()
            
            # Update model statistics and collect data
            self._update_model_statistics()
            self.datacollector.collect(self)
            
            # Check if this is the last step of the year for logging
            if (self.schedule.steps % self.steps_per_year == 0):
                self._log_annual_summary()
                # At the end of the year, all agents should have their annual counters reset for the new year.
                # The agent.step() method handles this reset, but let's confirm the timing.
    ...
    def _log_annual_summary(self) -> None:
        """Log annual summary statistics"""
        try:
            # The steps start from 1, so the year is steps // steps_per_year
            year = 2024 + (self.schedule.steps // self.steps_per_year)
            
            # Get the agent data from the last completed year
            last_year_data = self.datacollector.get_agent_vars_dataframe().xs(
                (self.schedule.steps-1), level="Step"
            )

            total_emissions = last_year_data['Annual_Emissions'].sum()
            total_production = last_year_data['Annual_Production'].sum()
            
            # Total abatement can be a cumulative value. Using the cumulative from the last step.
            total_abatement = last_year_data['Abated_Tons_Cumulative'].sum()
            ...
            logger.info(f"Compliance Rate: {self.model_stats['compliance_rate']:.1%}")
            logger.info(f"Total Trades: {len(self.market.transactions_log)}")
            logger.info(f"Market Volume: {self.model_stats['total_volume']:.2f} credits")
            logger.info(f"Market Maker Cash: ₹{self.market.market_maker_cash:.2f}")
            logger.info(f"Market Maker Reserve: {self.market.market_maker_reserve:.2f} credits")
            logger.info("=" * 30)
    ...
    #     try:
    #         year = 2024 + (self.schedule.steps // self.steps_per_year)
    #         total_emissions = sum(getattr(a, 'total_annual_emissions', 0) for a in self.schedule.agents)
    #         total_production = sum(getattr(a, 'total_annual_production', 0) for a in self.schedule.agents)
    #         avg_intensity = safe_divide(total_emissions, total_production)
    ...
    #         logger.info(f"Total Annual Emissions: {total_emissions:.2f} tonnes")
    #         logger.info(f"Total Annual Production: {total_production:.2f} units")
    #         logger.info(f"Total Abatement: {self.model_stats['total_abatement']:.2f} tonnes")
    #         logger.info(f"Average Intensity: {avg_intensity:.4f} tonnes/unit")
    #         logger.info(f"Compliance Rate: {self.model_stats['compliance_rate']:.1%}")
    #         logger.info(f"Total Trades: {self.model_stats['total_trades']}")
    #         logger.info(f"Market Volume: {self.model_stats['total_volume']:.2f} credits")
    #         logger.info(f"Market Maker Cash: ₹{self.market.market_maker_cash:.2f}")
    #         logger.info(f"Market Maker Reserve: {self.market.market_maker_reserve:.2f} credits")
    #         logger.info("=" * 30)
